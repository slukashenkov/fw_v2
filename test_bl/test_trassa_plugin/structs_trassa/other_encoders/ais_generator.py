...
def test_this():
    ...
    print
    "nav status: %d" % aismsg.get_attr("status")
    ...
    print
    "nav status: %d" % aismsg2.get_attr("status")
    ...
    aismsg = AISStaticDataReportAMessage(mmsi=237772000, shipname=AISString2Bits('OF THE HIGH SEAS').int)
    aismsg = AISStaticDataReportAMessage(mmsi=237772000, shipname='OF THE HIGH SEAS')
    ais = AIS(aismsg)
    payload = ais.build_payload(False)
    print
    payload
    ...
    #
    aismsg = AISStaticDataReportBMessage(mmsi=237772000, shiptype=36,
                                                vendorid='DIY',
                                                callsign='SVXYZ',
    ...
    print
    ' Tests for Message Type 5'
    aismsg = AISStaticAndVoyageReportMessage(mmsi=237772000,
                                                    imo=0,
                                                    callsign='SVXYZ',
...
def test_this_02():
    ...
    import time
    import sys
    ships = {'kornaros': (239311000, 'kornaros.txt',
                          '!AIVDM,1,1,,A,53T>HV01hAoM=PqP001HUA<DqA`u>0du8p58u<0t891::40Ht;@00000000000000000000,2*04'),
             'highspeed4': (239658000, 'highspeed4.txt',
                            '!AIVDM,1,1,,A,53TST402<`;M=L`@000PTLQ=0DDB3@000000000`5hf<<40Ht7P00000000000000000000,2*1D'),
             'venizelos': (237628000, 'venizelos.txt',
                           '!AIVDM,1,1,,A,53RWbH01pbEU=MM`000DjqHDpU`Dhu<00000000t:iF>>40HtAP00000000000000000000,2*58'),
             'knossos': (237641000, 'knossos.txt',
                         '!AIVDM,1,1,,A,53R`M:02<LEu=U4t000dpu=<u>104h4<D000000t=Ic==40HtA000000000000000000000,2*2D')}
